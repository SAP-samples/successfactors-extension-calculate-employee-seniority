module.exports = async (srv) => {
  ...
  const nodeRequestsCounter = new prometheus.Counter({
    name: 'service_handlers_requests',
    help: 'total service handlers requests'
  });
  
  // latency - histogram
  const nodeLatenciesHistogram = new prometheus.Histogram({
      name: 'service_handler_request_latency',
      help: 'service handler request latency by path',
      labelNames: ['route'],
      buckets: [100, 400]
  });
  ...
  srv.on("READ", "Photo", async (req) => {
    ...
    const result = await pUser.read("Photo", columns).limit(limit).where(combinedPred);
    const responseLatency = new Date().getTime() - requestReceived;
    nodeLatenciesHistogram
        .labels('/Photo')
        .observe(responseLatency);
  ...
  messaging.on(
    "sap/sfsf/SFPART057671/isc/contractchange",
    async (msg) => {
